...
- `includeDynamicRoutes` now works on VitePress 2. Its `resolvePages` no longer takes a source directory and returns the pages it found; it takes a site configuration and fills the pages into it instead. The old call raised a `TypeError` that was caught and reported as a loader failure, so every page generated by a dynamic route was dropped from the sidebar with nothing but a warning to explain it. The call is now chosen from the major version of the VitePress the project has installed, and both majors are covered by the test suite.
- The pages of a dynamic route are ordered by their path on VitePress 2, instead of in the order the `paths` loader returned them. VitePress 2 sorts them before handing them over and does not expose the original order, so a project that relied on the order of its `paths` loader has to state the order it wants, with an `order` parameter and `sortMenusByFrontmatterOrder`. Nothing changes on VitePress 1.
