---
order: 2
description: 'VitePress Sidebar의 모든 옵션을 기본값, 타입, 예시와 함께 정리했습니다. 경로, 제목, 링크, 정렬, 필터, 그룹 옵션을 다룹니다.'
---
...
값이 `true`이면 `.md` 파일의 `h1` 제목 내용이 포함된 제목을 표시합니다. 파일에 `h1` 제목이 존재하지 않으면 기존과 같이 파일 이름으로 대체합니다. `Frontmatter` 블록에서는 제목을 찾지 않으므로 그 안에 작성된 주석은 제목으로 사용되지 않으며, 셸 예제의 주석처럼 코드 블록 안에 있는 `#` 역시 제목으로 사용되지 않습니다.
...
정규식을 사용할 수도 있습니다. 정규식과 일치하는 첫 번째 값이 제거되므로, 구분자를 포함해 접두사 전체와 일치하는 정규식을 작성하세요. 예를 들어 `2024-01-01-hello`에서 문자열 앞의 날짜를 제거하려면 `prefixSeparator` 값을 `/[0-9]{4}-[0-9]{2}-[0-9]{2}-/g`로 지정합니다. 결과는 `hello`입니다.

문자열 형태로는 표현할 수 없는, 구분자를 포함한 접두사를 제거할 때 사용합니다. `/^[0-9]+\./`는 `1.1.hello`를 `1.hello`로 만들며(이는 `.`만 지정한 것과 같습니다), `/^[0-9]+\.[0-9]+\./`는 `hello`로 만듭니다.
...
루트 그룹은 `rootGroup` 옵션 중 하나를 설정한 경우에만 생성되며, `rootGroupText`로 이름을 지정하지 않으면 `Table of Contents`가 사용됩니다.
